...
public class AdminController {
    ...
    ResponseEntity<List<String>> findAllServices(){
        return new ResponseEntity<>(serviceService.findAll().stream().map(s -> s.getName()).toList(),HttpStatus.OK);
    }
    @GetMapping("/show-all-sub-service")
    ResponseEntity<List<String>> findAllSubServices(){
        return new ResponseEntity<>(subServiceService.findAll().stream().map(s -> s.getName()).toList(),HttpStatus.OK);
    }
